...
class LSH:
    ...
    def query(self, data_point, max_results, filename):
        retrieval = set()
        n_buckets = 0
        n_candidates = 0
        permLevel = 0
        ...
        result = {'n_buckets': n_buckets, 'n_candidates': len(retrieval), 'scores': sim_scores[data_idx].tolist(),
                  'retrieved_files': [self.idx_2_file[retrieval[d]] for d in data_idx],
                  'retrieved_idx': [retrieval[d] for d in data_idx]}
        json.dump(result, open(self.output_dir + "/{}_similar_{}_{}_{}_{}.txt".format(filename, self.L, self.k, self.vm, self.uc), "w"),
                  indent="\t")
        return result
...
if __name__ == "__main__":
    ...
    queryFile = input("Enter the query file name : ")
    qf = lsh.file_2_idx[str(queryFile).zfill(7)]
    maxResults = 10
    lsh.query(np.array(data[qf]), maxResults, queryFile.zfill(7))