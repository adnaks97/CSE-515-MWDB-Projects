...
class wordExtractor2(wordExtractor):
    def __init__(self, fname, component, bands=4, window_size=10, stride=3, DIR='Data/', out_dir="outputs"):
        super().__init__(fname, bands, window_size, stride, DIR)
        self.component = component
        self.DIR = self.DIR + '/' + self.component + '/'
        self.out_dir = os.path.abspath(out_dir)
        Path(os.path.join(self.out_dir,'task0a')).mkdir(parents=True, exist_ok=True)
        self.save_path = os.path.join(self.out_dir,'task0a','{}.wrd'.format(self.nfname))
